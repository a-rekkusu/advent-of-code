export function executePart2(input: string[]): number {
  ...
  for (const record of records) {
    ...
    if (problemIndex) {
      ...
      if (problemIndex !== 2 && problemIndex !== 1) isMutationSafe = isRecordSafeMutation(record, problemIndex - 1)
...
function isRecordSafeMutation(record: string[], problemIndex: number): boolean {
  ...
  for (let j = startIndex; j < record.length; j++) {
    let previous = +record[j - 1]
    if (j === problemIndex + 1) previous = +record[problemIndex === 0 ? 0 : problemIndex - 1]
...
): { isAscending: boolean | null; isSafe: boolean } {
  if (isAscending) {
