...
function prepareData(input: string[]): Update[] {
  ...
  input.forEach((line) => {
    ...
    if (line.length > 2 && line[2] === '|') {
      ...
      const rulesIndex = rules.findIndex((rule) => rule.pageNumber === pageNumber)
      ...
      const updateNumbers = line.split(',').map((updateNumber) => Number(updateNumber))
      updates.push({
        pages: updateNumbers,
  ...
  updates.forEach((update) => {
    ...
    rulesByKey.forEach((rule) => {
      if (rule.mustBeBefore.filter((x) => update.pages.includes(x)).length) update.rulesToApply.push(rule)
    })
  })
...
function solve(input: string[]): number {
  ...
  const updatesToCorrect: Update[] = []
  const correctedUpdates: Update[] = []

  // populate invalid updates to correct
  updates.forEach((update) => {
    for (let i = 0; i < update.pages.length; i++) {
      ...
      for (let j = 0; j < i; j++) {
        ruleToApply.mustBeBefore.forEach((before) => {
          if (update.pages[j] === before) {
            updatesToCorrect.push(update)
            // correct updates
            const correctedUpdate = update
            // fix the sorting somehow...
  ...
  return updatesToCorrect.reduce((acc, val) => acc + val.middleNumber, 0)
}
